...
function makeCode(){let code;do{code='WDR-'+Math.random().toString(36).substring(2,7).toUpperCase()}while(JSON.parse(localStorage.getItem('wander_bookings')||'[]').some(x=>x.code===code));return code}
if(form){if(key&&travelPackages[key])document.getElementById('package').value=travelPackages[key].title;const date=document.getElementById('date');if(date){const t=new Date();t.setDate(t.getDate()+1);date.min=t.toISOString().split('T')[0]}form.addEventListener('submit',e=>{e.preventDefault();const data={code:makeCode(),name:document.getElementById('name').value.trim(),phone:document.getElementById('phone').value.trim(),email:document.getElementById('email')?.value.trim()||'',package:document.getElementById('package').value,origin:document.getElementById('origin').value,guests:Number(document.getElementById('guests').value),date:document.getElementById('date').value,status:'Menunggu konfirmasi',createdAt:new Date().toISOString()};if(!/^08[0-9]{8,13}$/.test(data.phone.replace(/\D/g,''))){document.getElementById('status').textContent='Nomor WhatsApp belum valid. Gunakan format 08xxxxxxxxxx.';return}const all=JSON.parse(localStorage.getItem('wander_bookings')||'[]');all.push(data);localStorage.setItem('wander_bookings',JSON.stringify(all));document.getElementById('status').innerHTML='Booking berhasil dibuat. Kode booking kamu: <strong>'+data.code+'</strong>. Simpan kode ini untuk cek status.';form.reset()})}
const cb=document.getElementById('checkBtn');if(cb)cb.addEventListener('click',()=>{const code=document.getElementById('checkCode').value.trim().toUpperCase(),all=JSON.parse(localStorage.getItem('wander_bookings')||'[]'),b=all.find(x=>x.code===code),r=document.getElementById('checkResult');r.innerHTML=b?'<strong>'+b.code+'</strong><br>'+b.package+' · '+b.guests+' orang · '+b.date+'<br>Status: <b>'+b.status+'</b>':'Booking tidak ditemukan. Periksa kembali kode booking.'});
function renderAdmin(){const rows=document.getElementById('bookingRows');if(!rows)return;const all=JSON.parse(localStorage.getItem('wander_bookings')||'[]');document.getElementById('totalBookings').textContent=all.length;document.getElementById('pendingBookings').textContent=all.filter(x=>x.status==='Menunggu konfirmasi').length;document.getElementById('totalGuests').textContent=all.reduce((a,x)=>a+x.guests,0);rows.innerHTML=all.map(x=>'<tr><td><b>'+x.code+'</b></td><td>'+x.name+'<br><small>'+x.phone+'</small></td><td>'+x.package+'</td><td>'+x.date+'</td><td>'+x.guests+'</td><td>'+x.status+'</td></tr>').join('')||'<tr><td colspan="6">Belum ada booking.</td></tr>'}
renderAdmin();document.getElementById('clearBookings')?.addEventListener('click',()=>{if(confirm('Hapus semua booking di browser ini?')){localStorage.removeItem('wander_bookings');renderAdmin()}});
